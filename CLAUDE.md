...
- The app, widgets, and share extension share an App Group: `group.io.github.assassin808.wanderly` on iOS and `ZS2UVBACK8.io.github.assassin808.wanderly` on macOS. On macOS, `group.` containers are TCC-protected: a signature mismatch shows "would like to access data from other apps" and blocks the calling thread, which froze app launch. Keep `SharedContainer.appGroup` and the `-macOS.entitlements` files in sync. `SharedContainer.defaultURL` is nil in unsigned builds, so every caller has to handle that.
